-- Elite Bet Supabase Database Schema - ULTRA SAFE VERSION
-- Run this in your Supabase SQL Editor to fix the infinite recursion issue
-- This version adds missing columns first, then handles everything else
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'status') THEN
        ALTER TABLE public.users ADD COLUMN status TEXT DEFAULT 'active' CHECK (status IN ('active', 'suspended', 'closed'));
    END IF;
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'risk_level') THEN
        ALTER TABLE public.users ADD COLUMN risk_level TEXT DEFAULT 'low' CHECK (risk_level IN ('low', 'medium', 'high'));
    END IF;
    ...
    -- Update transaction type constraint to include 'fee'
...
-- Create new RLS Policies for transactions table
...
-- Create new RLS Policies for audit_logs table (simplified - no admin check)
CREATE POLICY "Users can view own audit logs" ON public.audit_logs
  FOR SELECT USING (auth.uid() = user_id);

-- Create new RLS Policies for games table (public read)
CREATE POLICY "Anyone can view active games" ON public.games
  FOR SELECT USING (status = 'active');

-- Create new RLS Policies for bets table
...
-- Insert some sample data (only if not exists)
